/**
 * disableLawyer — Admin-only. Disables a lawyer account. Sends notification email.
 */
...
function buildDisabledEmail(firstName) {
  return `<!DOCTYPE html>
<html lang="en">
  ...
  <tr><td align="center">
    <table width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;">
      ...
      <tr><td style="background:#ffffff;border-radius:16px;padding:40px 48px;box-shadow:0 2px 16px rgba(0,0,0,0.08);">
        <h1 style="margin:0 0 8px;color:#111827;font-size:26px;font-weight:700;">Account Access Update</h1>
        ...
        <p style="margin:0 0 16px;color:#333333;font-size:15px;line-height:1.7;">Your access to the Taylor Made Law Network has been temporarily disabled.</p>
        <p style="margin:0 0 16px;color:#333333;font-size:15px;line-height:1.7;">If you believe this is an error, please contact us at <a href="mailto:support@taylormadelaw.com" style="color:#3a164d;">support@taylormadelaw.com</a>.</p>
      </td></tr>
...
Deno.serve(async (req) => {
  try {
    ...
    if (resendKey) {
      ...
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: [lawyerUser.email],
          subject: 'Account Access Update — Taylor Made Law',
          html: buildDisabledEmail(firstName)
        })
      });
      emailSent = res.ok;
    }
    ...
    await base44.asServiceRole.entities.AuditLog.create({
      entity_type: 'User',
      entity_id: user_id,
      action: 'lawyer_disabled',
      actor_email: adminUser.email,
      actor_role: 'admin',
      notes: `Disabled by ${adminUser.email}. Reason: ${reason || 'none'}`
    });

    return Response.json({ success: true, email_sent: emailSent });
  } catch (error) {
    console.error('disableLawyer error:', error);
