...
Deno.serve(async (req) => {
  try {
    ...
    if (isActivated) {
      ...
      // Email is NOT passed in the URL — it is resolved server-side from the token on the SetPassword page.
      const activationUrl = `${BASE_URL}/SetPassword?token=${rawToken}`;
